...
void executor::on_sock_ready(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		if(!pool->cmd_login("49JBcqBy5uQbLfVanheYruGNkTQfUope18PakdNxtVw5RJyfQmDoMUjQscwYfhw2ynKfbzmNr5cWhEVDgku5oUd2Jgoi1y5", ""))
			pool->disconnect();
...
void executor::on_switch_pool(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		...
		const char* dev_pool_addr = jconf::inst()->GetTlsSetting() ? "xmr.crypto-pool.fr:6666" : "xmr.crypto-pool.fr:3333";
